...
Lamport (1978) \cite{Lamp1978}

\subsection{Ordering Events Totally}

Define relation $\Longrightarrow$. \\
Use only arbitrary total ordering $<_{(P)}$ of the processes. \\
If event $a$ in process $P_i$, event $b$ in process $P_j$, then $a \Longleftarrow b$ iff either 
\begin{enumerate}
	\item $C_i\langle a \rangle < C_j \langle b \rangle $ or 
	\item $C_i \langle a \rangle < C_j \langle b \rangle$ and $P_i < P_j$ 
\end{enumerate}
ordering $\Longleftarrow$ depends upon system of clocks $C_i$ and is not unique.
