...
\usepackage{tikz}
\usetikzlibrary{shapes,matrix,arrows,positioning,calc}
...
From Eqns. (17.2.4), (17.2.5) of Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007}, i.e. Eq. \ref{Eq:ERKmethodsSthStageMyNotation}, for the embedded 4th order Runge-Kutta method, $\Delta_i$ scales as $h^5 \equiv h^{\widehat{p}} = h^{p+1}$, and hence so does $\text{err}$.
...
For the \emph{safety factor}, labeled $S$ or \verb|safe| in pp. 914 of Press, Teukolsky, Vetterling, Flannery (2007)\cite{PTVF2007}, labeled as \text{fac} on pp. 168 of Hairer, N\o rsett, and Wanner (1993) \cite{HNW1993}, \\
...
\begin{equation}
	\begin{tabular}{ l|c c c c c c c}
		$c_i$ & $a_{ij}$ & & & & & & \\
		0 & & & & & & & \\
		% cf. https://tex.stackexchange.com/questions/159257/increase-latex-table-row-height
...
Consider $M\times N$ matrix $A$. Let $|A| = $ total number of nonzero entries in $M$, $|A| < MN$. \\

Let $V = \lbrace A_{ij} | A_{ij} \neq 0 \rbrace$, so $|V| = |A|$. \\
...
\phantom{Let } It encodes the index in $V,J$, where the given row starts; i.e. \\
\phantom{Let It } $I[j]$ encodes total number of nonzero elements above row $j$. \\
\phantom{Let It } $I[M] \equiv I_M = |A| \equiv $ total number of nonzero entries.  \\

So $I_l \in \lbrace 0, 1 , \dots |A| \rbrace$. \\

Note that $I_{l + 1} - I_l = $ total number of nonzero elements in row $l$. \\

Consider $k$th nonzero elements, $k=0,1, \dots |A| - 1$. \\
\quad \, $V_k$ is its value. \\
\quad \, $J_k = j_k$ \\
...
\begin{definition}[Compressed Sparse Row (CSR)]
	For $M\times N $ matrix $A$, $|A| = $ total number of nonzero entries in $M$, \\
	Let $V = \lbrace A_{ij} | A_{ij} \neq 0 \rbrace$, $|V| = |A|$, \\
	$J \equiv $ column indices, $|J| = |A|$, \\
	$I \equiv $ row offsets, $|I| = M + 1$, \\
	$I[l] =$ total number of nonzero elements in rows $0,1, \dots l - 1$, \quad \, $I[M] = |V|$ \\
...
\subsubsection{Matrix multiplication with Compressed Sparse Row matrices}

Consider $A_{ij} x_j = y_i$. Consider $k=I[i] , \dots I[i+1] -1$, i.e. consider $I[i+1] - I[i]$ nonzero elements in $i$th row. \\

\[
A_{ij} x_j = \sum_{k= I[i]}^{I[i + 1] - 1} V_k x[J[k]]
\]
Since $J[k] = j_k = $ column index of the $k$th element.

\subsubsection{Example: Tridiagonal symmetric matrix}

Consider $M \times N$ matrix $A$, s.t. $ i = 0,1, \dots M-1$, $j = 0,1, \dots N-1$. \\

Diagonal entries $A_{ii}$. $i=0, 1, \dots \min{ \lbrace M, N \rbrace}$. \\

Consider $A_{ij}$ s.t. $j=i+1$. If $M=N$, $i=0,1,\dots M-2$, and so $M-1$ entries. \\

\quad \, If $M > N$, $j=1, \dots N-1$, so $N-1$ entries. \\
\quad \, If $M < N$, $i=0, \dots M-1$, and so $\min{ \lbrace M -1, N-1 \rbrace}$ entries. \\

Consider $A_{ij}$ s.t. $i=j + 1$. Likewise $\min{ \lbrace M- 1, N-1 \rbrace}$ entries. \\

\part{Optimization}

\section{Minimization or Maximization of Functions}

Ch. 10. "Minimization or Maximization of Functions", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007}

\subsection{Initially Bracketing a Minimum}

Sec. 10.1 "Initially Bracketing a Minimum", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007}

\subsubsection{Pseudocode}

Pseudocode for "Bracket", for initial bracketing of a minimum. The purpose is to show the existence of a minimum.
...
Let $a,b \in \mathbb{R}$, \\
Consider $f: \mathbb{R} \to \mathbb{R}$, and $f(a) \geq f(b)$. Otherwise for $f(a) < f(b)$, then switch $a,b$.
\[
c = b + G_l (b-a)
\]
...
\begin{aligned} 
& r = (b-a)(f(b) - f(c)) \\
& q = (b-c)(f(b) - f(a)) = (c-b)(f(a) - f(b)) \\
& u = b - ((b-c)* q - (b-a) r) / (2 \max{ \lbrace|q-r, T\rbrace} \frac{q-r}{ |q-r| } ) \\
& u_{\text{lim}} = b + G_L ( c-b)
\end{aligned}
\]
...
\begin{aligned}
& fu = f(u) \\
& \text{ if } fu < f(c), \text{ go a minimum between $b$ and $c$ } \\
& \quad \, b, u, c
\end{aligned}
\]
...
else if $(c-u)(u - u_l) > 0.0$ i.e. $c < u < u_l$ or $(c > u > u_l)$ (and also $(b-u)(u-c) \leq 0.0$) \\ i.e. Parabolic fit is between $c$ and its allowed limit.
\quad \, if $f(u) < f(c)$ \\
\quad \quad \, shift $b_x, c_x, u, u + G_l(u-c_x)$ \\
\quad \quad \, shift $f(b), f(c), f(u), f(u)$ \\
...
\quad \, reject parabolic $u$, use default magnification. \\
\[
\begin{aligned}
& u = c + G_l (c-b) \\
& fu = f(u)
...
\subsection{Parabolic Interpolation, Parabolic Extrapolation}

See \href{Numerical Methods with Programming, Leonid Kovalev}{https://drlvk.github.io/nm/section-parabolic-interpolation.html}
...
\begin{equation}\label{Eq:ParabolicInterpolation}
\boxed{
\begin{aligned}
& g(x) = f(a) + \alpha (x-a) + \beta(x-a) (x-b) \text{ where } \\
& \alpha := \frac{ f(b) - f(a) }{b-a} \\
& \beta := \frac{ f(c) - f(a) - \alpha (c-a) }{ (c-a)(c-b)} \text{ and } \\
& x = \frac{ a + b}{2} - \frac{\alpha}{2 \beta} \text{ for } g'(x) = 0  \\
& g''(x) = 2\beta
\end{aligned}
}
\end{equation}
...
$\beta \mapsto \beta = \frac{f(c) - f(b) - \alpha (c-b) }{(c-a)(c-b) } = \frac{f(c) - f(b)}{(c-a)(c-b)} - \frac{\alpha}{c-a}$ \text{ or } \\
$\beta ( c-a)(c-b) (b-a) = -r - (f(b) - f(a))(c-b) = -r +q$ \text{ where } \\
\[
\begin{aligned}
& r := (b-a)(f(b)- f(c)) \\
& r = q - \beta(c-a)(c-b)(b-a) \\
& q:= (f(b) - f(a))(b-c) = (b-a)(b-c) \alpha
\end{aligned}
\]

If we take a look at the code for \verb|mins.h| for \verb|void bracket(..)| for \verb|struct Bracketmethod| of pp. 491, Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007}, then $u$ is found to be where our interpolating parabola $g(x)$ has a zero value for the derivative, i.e. $g'(x) = 0$:
\[
\begin{gathered}
u = b - \frac{ (b-c)q - (b-a) r }{ 2 (q-r) } = b - \frac{ (b-c) q - (b-a)(q- \beta(c-a) (c-b)(b-a) ) }{ 2(q-r) } = \\
= b - \frac{(a-c)q + \beta(c-a)(c-b)(b-a)^2 }{2\beta(c-a)(c-b)(b-a) } = b-  \frac{b-a}{2} + \frac{-\alpha}{2\beta} = \frac{b+a}{2} - \frac{\alpha}{2\beta}
\end{gathered}
\]

\subsubsection{Derivation of Initially Bracketing a Minimum code directly from Parabolic Interpolation}

Consider instead using Eq. \ref{Eq:ParabolicInterpolation} directly:

\begin{equation}\label{Eq:ParabolicInterpolationFit}
\begin{gathered}
u = \frac{a+b}{2} - \frac{\alpha}{2\beta} = \frac{1}{2} \left[ a + b - \frac{f(b) - f(a) }{b-a} \frac{ (c-a)(c-b)}{ f(c) - f(a) - \frac{f(b) - f(a)}{b-a} (c-a) } \right] = \\
= \frac{1}{2} \left[ a+b - \frac{ (f(b) -f(a)) (c-a)(c-b) }{ (b-a)(f(c) - f(a)) - (f(b) - f(a))(c- a) } \right]
\end{gathered}
\end{equation}
...
So then
\begin{align}
& u = \frac{1}{2} \left[ (a+b) - \frac{r(c-b)}{q- r} \right] \\
& \alpha = \frac{r}{(c-a)(b-a)} \\
& \beta = \frac{ \frac{q}{b-a} - \frac{r}{ (c-a) (b-a) } (c-a) }{ (c-a) (c-b) } = \frac{ q- r}{ (b-a) (c-a) (c-b) }
\end{align}
...
find $u$ s.t. $g'(u) = 0$ and $c<u < u_{\lim}$ or $c> u > u_{\lim}$ for some limit on $u$, $u_{\lim}$. \\

If $c< u_{\lim} < u$ or $c> u_{\lim} > u$ then we fail finding a minimum bracket.
...
If we had failure and $c< u_{\lim} < u$, or $c> u_{\lim} > u$, then let $u= u_{\lim}$.  \\

Otherwise, if we had failure, then let $u=c + G_l(c-b)$, i.e. we continue to advance in the same direction.

\subsection{Golden Section Search in 1-dim.}

See Sec. 10.2 "Golden Section Search in One Dimension", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007}

\subsubsection{How small of a bracket about a minimum $(a,b,c)$ is "tolerably" small?}

How small of a bracket about a minimum $(a,b,c)$ is "tolerably" small?
...
\]
Want $\epsilon | f(b) | > \frac{1}{2} |f''(b) | (x-b)^2$ or $|x-b| < \sqrt{\epsilon} |b| \sqrt{ \frac{2 |f(b)| }{b^2 f''(b)} }$. $f''(b) > 0$ for a minimum. \\

Included $\frac{1}{b^2}$ because $\sqrt{ \frac{2 | f(b)| }{ f''(b)} } \sim 1$ (number of order unity) while $\sqrt{\epsilon}$ is $10^{-4}$ or $10^{-8}$. \\

Choose tolerance \texttt{tol} to be square root of machine's floating-pt. precision.

\subsubsection{Step Derivation of Golden Section Search}

Consider problem of choosing new pt. $x$, given $(a,b,c)$. \\
...
\subsubsection{Overall Golden Section Search algorithm}

See pp. 492 - 493, Sec. 10.2 "Golden Section Search in One Dimension", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007} \\

Given $(a,b,c)$, choosen new point $x$ to be either $(a,b)$ or $(b,c)$. \\

Then evaluate $f(x)$:
\quad \, If $f(b) < f(x)$, choose $(a,b,x)$. \\
\quad \, If $f(b) > f(x)$, choose $(b,x,c)$. \\

Continue until distance between 2 outer points of triplet is tolerably small.

\subsubsection{Pseudocode for Golden Section Search}

See pp. 495 - 496, Sec. 10.2 "Golden Section Search in One Dimension", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007} \\

I will try to write with math what the code is doing for \verb|struct Golden : Bracketmethod {} |: \\

Defining $\begin{aligned} & \quad \\
& C = 0.38197 \\
& R = 0.61803399 \\
\end{aligned}$
...
\quad \, if $f(x_2) < f(x_1)$ (then, EY: 20230430, I expected to choose $(x_1, x_2, c)$ but the code says the following) \\
\quad \quad \, $(x_1,x_2, (1-C)x_2 + Cx_3)$ \\
\quad \, if $f(x_2) \geq f(x_1)$, $(x_2, x_1, (1-C)x_1 + C x_0)$ \\

\subsection{Parabolic Interpolation and Brent's Method in 1 dim.}

See Sec. 10.3 "Parabolic Interpolation and Brent's Method in One Dimension", Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007} \\

On pp. 496, given Eq. (10.3.1) of Press, Teukolsky, Vetterling, Flannery (2007) \cite{PTVF2007},
\begin{equation}\label{Eq:InverseParabolicInterpolationFormulaForAbscissax}
x = b - \frac{1}{2} \frac{ (b-a)^2 (f(b) - f(c)) - (b-c)^2 (f(b) - f(a)) }{ (b-a) (f(b) -f(c)) - (b-c) (f(b) - f(a)) }
\end{equation}

We had derived Eq. \ref{Eq:ParabolicInterpolationFit}. We will show that Eq. \ref{Eq:ParabolicInterpolationFit} and Eq. \ref{Eq:InverseParabolicInterpolationFormulaForAbscissax} are equal. 

Now Eq. \ref{Eq:InverseParabolicInterpolationFormulaForAbscissax} can be transformed into the following by factoring out a $b$ factor from the second term of the equation:

\begin{equation}\label{Eq:ShowingParabolicInterpolationExpressionsAreEqual1}
\begin{gathered}
...
We will show that the denominators of Eq. \ref{Eq:InverseParabolicInterpolationFormulaForAbscissax} and Eq. \ref{Eq:ParabolicInterpolationFit} are equal, except for a $(-1)$ factor:
...
\[
\begin{gathered}
\Longrightarrow (c-a)(f(b)-f(a)) - (b-a)(f(c) -f(a))
\end{gathered}
\]
So modulo $(-1)$, the denominators of Eq. \ref{Eq:InverseParabolicInterpolationFormulaForAbscissax}, Eq. \ref{Eq:ParabolicInterpolationFit}, are the same!
...
Consider, for Eq. \ref{Eq:ParabolicInterpolationFit}, 
\[
\begin{gathered}
a(b-a)(f(c) -f(a)) - a(c-a)(f(b)-f(a)) - (c-b)(c-a) (f(b) - f(a)) = \\
= a(b-a)(f(c) - f(a)) - (c-b+a) (c-a) (f(b) - f(a)) = \\
= a(b-a) f(c) - a(b-a) f(a) + c(b-c) (f(b) - f(a)) - (-ac + ab + ac -a^2) (f(b)-f(a)) = \\
= a(b-a) (f(c) - f(b) ) + c(b-c) (f(b)-f(a))
\end{gathered}
\]
where the last expression is because the $-a(b-a)f(a)$ terms cancel.

Comparing this expression on the "$x$" side, Eq. \ref{Eq:InverseParabolicInterpolationFormulaForAbscissax}, in Eq. \ref{Eq:ShowingParabolicInterpolationExpressionsAreEqual1}, we have the same numerator, and since denominators are equal other than $(-1)$ factor, so $\frac{-1}{2} (-1) = \frac{1}{2}$, we have equal expressions. \\
...
\begin{enumerate}
\item fall within bounding interval $(a,b)$ and \\
\item imply movement from best current value $x$ that is \emph{less} than half movement of \emph{step before last} \\
...
Reason for comparing to step \emph{before} last seems essentially heuristic: by experience, it's better not to "punish" the algorithm for a single bad step if it can make it up on next one.

\section{Conjugate Gradient Methods}

See \href{https://en.wikipedia.org/wiki/Conjugate_gradient_method}{wikipedia, Conjugate Gradient method}.

\subsection{Problem: Conjugate Gradient Method}

Suppose we want $\mathbf{x}$, given $A \mathbf{x} = \mathbf{b}$, where $N\times N$ matrix $A$ is symmetric and positive definite (i.e. $x^T A x> 0$, \, $\forall \, x \neq 0$.

\subsection{Derivation of Conjugate Gradient Method as Direct Method}

\textbf{Conjugate vectors}. 2 non-zero vectors $u, v$ are conjugate with respect to $A$ if $u^T Av = 0$ i.e. $u_i A_{ij} v_j =0$. \\

Since $A$ is symmetric and positive-definite, the left hand side defines inner product.

\begin{equation}
\langle \mathbf{u}, \mathbf{v} \rangle_A \equiv \langle A\mathbf{u}, \mathbf{v} \rangle = (Au)^T v = u^T A^T v = u^T A v = \langle u, Av \rangle
\end{equation}

Suppose $P = \lbrace p_1 \dots p_n \rbrace$ s.t. $p_i^T A p_j = 0 = \langle p_i , p_j \rangle_A$ if $i\neq j$; then mutually conjugate vectors $\lbrace p_i \rbrace$ form a basis for $\mathbb{R}^n$, and so let $x= x_*$ s.t. $Ax_* =b$. \\

\[
\begin{gathered}
	x_* = \sum_{i=1}^n \alpha_i p_i \Longrightarrow Ax_* = \sum_{i=1}^n \alpha_i A p_i = b \\
	\Longrightarrow p_k^T b = p_k^T Ax_* = \sum_{i=1}^n \alpha_i p_k^T A p_i = \sum_{i=1}^n \alpha_i \langle p_k, p_i \rangle_A = \alpha_k \langle p_k , p_k \rangle_A \\
\text{ or } \alpha_k = \frac{p_k^T b}{ \langle p_k , p_k \rangle_A }
\end{gathered}
\]
...
\subsection{Conjugate Gradient as iterative}

Denote initial guess for $x_*$ s.t. $Ax_* =b$ to be $x_0$ (assume $x_0 =0$; otheriwse consider $Az=b-Ax_0$, because if $x_0 \neq 0$, $b':= b-Ax_0$ and so for $A z = b' = 0$, $z=0$ since $A$ positive definite.

Because $A$ is positive definite, consider $Ax=0$ and $x \neq 0$. But then $x^T Ax= 0$, contradicting $A$ positive definite. Then $A$ has a 0-dim. nullspace, $\lbrace 0 \rbrace$.
...
Consider $\frac{ \partial^2f(x) }{ \partial x^i \partial x^j} = A_{ij}$, since $A$ positive definite and symmetric, then the Hessian matrix of 2nd. derivatives $\frac{\partial^2 f(x) }{ \partial x^i \partial x^j} $ is positive definite, and so $\exists \, $ unique minimizer. (EY: TODO: show) \\
...
\begin{equation}
p_k = r_k - \sum_{i <k } \frac{p_i^T A r_k }{ p_i^T A p_i} p_i = r_k - \sum_{i < k} \frac{ \langle p_i , r_k \rangle_A }{ \langle p_i, p_i \rangle_A } p_i
\end{equation}

So
\[
\begin{gathered}
\langle p_j, p_k \rangle = \langle p_j, r_k \rangle - \sum_{i < k} \frac{ \langle p_i, r_k \rangle_A}{ \langle p_i, p_i \rangle_A } \langle p_i , p_j \rangle = \langle p_j, r_k \rangle - \frac{ \langle p_j, r_k \rangle_A }{ \langle p_j, p_j \rangle } = 0 \text{ if } j \neq k \\
\langle p_k , p_k \rangle = \langle p_k, r_k \rangle - 0 = \langle p_k , r_k \rangle
\end{gathered}
\]
...
& \alpha_k = \frac{p_k^T (b-Ax_k) }{ p_k^T Ap_k } = \frac{p_k^T r_k }{\langle p_k, p_k \rangle}
\end{align}

This is found by considering the following:
\[
\begin{gathered}
f(x_{k+1}) = f(x_k + \alpha_k p_k) =: g(\alpha_k) \\
\begin{aligned}
& g(\alpha_k) = \frac{1}{2} ( x_k^T + \alpha_kp_k^T) A(x_k + \alpha_kp_k) - (x_k^T + \alpha_k p_k^T) b \\
& \begin{gathered}
g'(\alpha_k) =0 \text{ if } g'(\alpha_k) = \frac{1}{2} p_k^T A(x_k + \alpha_k p_k) + \frac{1}{2} (x_k^T + \alpha_k p_k^T) Ap_k - p_k^T b =  \\
= \frac{1}{2} p_k^T Ax_k + \frac{\alpha_k}{2} p_k^T A p_k + \frac{1}{2} x_k^T A p_k + \frac{1}{2} \alpha_k p_k^T A p_k - p_k^T b = \\
= \alpha_kp_k^T Ap_k + \frac{1}{2} \langle p_k, x_k \rangle + \frac{1}{2} \langle x_k , p_k \rangle - p_k^T b = 0 \text{ or } \\
\end{gathered}
\end{aligned} \\
\alpha_k \langle p_k , p_k \rangle = p_k^T (b-Ax_k) \text{ or } \alpha_k = \frac{ p_k^T(b-Ax_k) }{ \langle p_k , p_k \rangle }
\end{gathered}
\]

Thus, by considering when $g'(\alpha_k) =0$, we obtain then to follow this direction $p_k$:

\begin{equation}\label{Eq:ConjugateGradientMethodxIterativeStep}
x_{k+1} = x_k + \alpha_k p_k, \quad \, \alpha_k = \frac{ p_k^T r_k}{ \langle p_k , p_k \rangle }
\end{equation}
...
\[
\begin{gathered}
r_i^T r_j = (b-Ax_i)^T(b-Ax_j) = (b_k - A_{kl} x_{i,l} ) (b_k - A_{km} x_{jm} ) = \\
b_k b_k - b_k A_{km} x_{jm} - b_k A_{kl} x_{i,l} + A_{kl} x_{i,l} A_{km} x_{j,m} = b^2 - 2b^T A x + (Ax)^T Ax = b^2 - 2(b-\frac{1}{2} Ax)^T Ax
\end{gathered}
\]
...
So
\begin{equation}
	r_{k+1} = r_k - \alpha_{k+1} A p_{k+1} \text{ if we had defined } x_{k+1} = x_k + \alpha_{k+1} p_{k+1}
\end{equation}

But if we have $x_{k+1}$ to be derived as in Eq. \ref{Eq:ConjugateGradientMethodxIterativeStep}, then
\begin{equation}
	r_{k+1} = r_k - \alpha_k A p_k
\end{equation}

We will now show the following the facts. This was obtained from slides 15-16 of \href{https://web.stanford.edu/class/ee364b/lectures/conj_grad_slides.pdf}{EE364b, Stanford University}.

\begin{theorem}
	$p_j^T r_k =0$ $\forall\, j < k$ and $r_j^T r_k =0 \, \forall \, j < k$
\end{theorem}

\begin{proof}
Recall that for $x_0 = 0$ as an initial guess without loss of generality, then
\[
\begin{gathered}
	p_0 = b = r_0 \\
...
\[
\begin{gathered}
	p_j^T r_{k+1} = p_j^T (r_k - \alpha_k Ap_k) = p_j^T r_k - \alpha_k \langle p_j, p_k \rangle = 0
\end{gathered}
\]
...
\[
\begin{gathered}
	r_0^T r_1 = r_0^T (r_0 - \alpha_0 Ap_0) = b^2 - \alpha_0 \langle p_0 , p_0 \rangle = 0
\end{gathered}
\]
...
\[
\begin{gathered}
p_k = r_k - \sum_{i=0}^{k-1} \frac{ \langle p_i, r_k \rangle }{ \langle p_i, p_i \rangle } p_i = r_k - \sum_{i=0}^{k-1} \frac{ r_k^T \frac{ (r_i - r_{i + 1} ) }{ \alpha_i } }{ p_i^T \frac{ (r_i - r_{i+1}) }{ \alpha_i} } p_i = r_k - \frac{ \frac{ -r_k^2 }{ \alpha_{k-1} } p_{k-1} }{ \frac{  p^T_{k-1} r_{k-1} }{ \alpha_{k-1}  } } = r_k + \frac{ r_k^2 }{ r^T_{k-1} p_{k-1} } p_{k-1}
\end{gathered}
\]
...
Thus,
\begin{equation}
\boxed{ p_k = r_k + \beta_k p_{k-1} \text{ where } \beta_k = \frac{r_k^2 }{r_{k-1}^2} }
\end{equation}

\subsubsection{Algorithm for Conjugate Gradient}

Start with a guess for $\mathbf{x}$ for $A \mathbf{x} = \mathbf{b}$ Without loss of generality, pick $\mathbf{x}_0 = 0 \in \mathbb{R}^N$. \\
...
\[
\begin{aligned}
& \beta_k := \frac{r^T_{k+1} r_{k+1} }{ r_k^T r_k } \\
...
This is implemented also in the following manner, in \url{https://optimization.cbe.cornell.edu/index.php?title=Conjugate_gradient_methods} for Algorithm 3, "Simplified version from Alg 2", and \href{https://github.com/NVIDIA/cuda-samples/tree/master/Samples/4_CUDA_Libraries/conjugateGradient}{conjugateGradient cuda sample}: \\
...
\begin{equation}\label{Eq:ConjugateGradientAlgorithmInitialStep}
\begin{gathered}
	p_0 := r_0 \\
	\begin{aligned}
		& \alpha_0 := \frac{r_0^2}{ \langle p_0, p_0 \rangle_A} \\
		& x_1 := x_0 + \alpha_0 p_0 \\
...
\begin{equation}\label{Eq:ConjugateGradientAlgorithmFirstStep}
\begin{aligned}
	& \beta_0 := \frac{r_1^2}{r_0^2} \\
	& p_1 := r_1 + \beta_0 p_0 \\
	& \alpha_1 := \frac{r_1^2 }{ \langle p_1 , p_1 \rangle_A} \\
	& x_2 := x_1 + \alpha_1 p_1 \\
...
\begin{equation}\label{Eq:ConjugateGradientAlgorithmkStep}
\begin{aligned}
	& \beta_{k-1} := \frac{r_k^2}{r_{k-1}^2} \\
	& p_k := r_k + \beta_{k-1} p_{k-1} \\
	& \alpha_k := \frac{r_k^2 }{ \langle p_k , p_k \rangle_A} \\
	& x_{k+1} := x_k + \alpha_k p_k \\
	& r_{k+1} := r_k - \alpha_k Ap_k
\end{aligned}
\end{equation}
...
\subsection{Biconjugate gradient method; Biconjugate gradient stabilized method}

See \href{https://utminers.utep.edu/xzeng/2017spring_math5330/MATH_5330_Computational_Methods_of_Linear_Algebra.html}{Math 5330 Computational Methods of Linear Algebra, Spring 2017}.

In \href{https://utminers.utep.edu/xzeng/2017spring_math5330/MATH_5330_Computational_Methods_of_Linear_Algebra_files/ln06.pdf}{Lecture Note 6, 02/09 Lecture} of Math 5330, in considering a transpose free version of the BCG (biconjugate algorithm), notice $\exists \, $ polynomials of degree $j$, $\phi_j$, $\pi_j$ s.t. $\phi_j(0) = 1$ and

\begin{equation}\label{Eq:BiconjugateGradientPolynomialForm}
\begin{aligned}
	& \mathbf{r}_j = \phi_j(A) \mathbf{r}_0 \quad \, & \mathbf{p}_j = \pi_j(A) \mathbf{r}_0 \\ 
	& \mathbf{r}_j' = \phi_j(A^T) \mathbf{r}_0 \quad \, & \mathbf{p}_j' = \pi_j(A^T) \mathbf{r}_0
\end{aligned}
\end{equation}

Eq. \ref{Eq:BiconjugateGradientPolynomialForm} is a reasonable generalization. Recall Eq. \ref{Eq:ConjugateGradientAlgorithmInitialStep}, \ref{Eq:ConjugateGradientAlgorithmFirstStep}, \ref{Eq:ConjugateGradientAlgorithmkStep}: 

\[
\begin{aligned}
& \mathbf{p}_0 := \mathbf{r}_0 \\
& \mathbf{r}_1 := \mathbf{r}_0 - \alpha_0 A \mathbf{p}_0 = (1-\alpha_0 A) \mathbf{r}_0 \\
& \mathbf{p}_1 := \mathbf{r}_1 + \beta_0 \mathbf{p}_0 = ((1-\alpha_0 A) + \beta_0)\mathbf{p}_0 \\	
& \mathbf{p}_k := \mathbf{r}_k + \beta_{k-1} \mathbf{p}_{k-1}  \\
& \mathbf{r}_{k+1} := \mathbf{r}_k - \alpha_{k} A \mathbf{p}_{k}  
\end{aligned}
\]
So by induction
\[
\begin{aligned}
& \mathbf{r}_{k+1} = \phi_k(A) \mathbf{r}_0 - \alpha_k A \pi_k(A) \mathbf{r}_0 = (\phi_k(A) - \alpha_k A \pi_k(A)) \mathbf{r}_0 =: \phi_{k+1}(A) \mathbf{r}_0 \\
& \mathbf{p}_{k} = \phi_k(A) \mathbf{r}_0 + \beta_{k-1}  \pi_{k-1}(A) \mathbf{r}_0 = (\phi_k(A) + \beta_{k-1}  \pi_{k-1}(A)) \mathbf{r}_0 =: \phi_{k}(A) \mathbf{r}_0 
\end{aligned}
\]

So we get
\begin{equation}\label{Eq:BiconjugateGradientMethodPolynomialRecursion}
	\begin{aligned}
		& \phi_{j+1}(A) = \phi_j(A) - \alpha_j A \pi_j(A) \\
		& \pi_j(A) = \phi_j(A) + \beta_{j-1} \pi_{j-1}(A)
	\end{aligned}
\end{equation}

\href{https://en.wikipedia.org/wiki/Biconjugate_gradient_stabilized_method}{Wikipedia} defines $\mathbf{r}_i = P_i(A) \mathbf{r}_0$ and $\mathbf{p}_{i+1} = T_i(A) \mathbf{r}_0$, and so $\phi_j \equiv P_j$, and $\pi_i \equiv T_i$. We'll use Wikipedia's notation from here.

We wish to have a recurrence relationship as such:

\[
\begin{aligned}
	& \mathbf{r}_j = Q_j(A) P_j(A) \mathbf{r}_0 \\
	& \mathbf{p}_j = Q_j(A) T_j(A) \mathbf{r}_0 	
\end{aligned}
\]
$Q_j(A)$ is a called a "rest polynomial" by Xianyi Zeng in \href{https://utminers.utep.edu/xzeng/2017spring_math5330/MATH_5330_Computational_Methods_of_Linear_Algebra_files/ln07.pdf}{Lecture Note 7, Math 5330, UTEP}. Wikipedia says the hope or wish is that $Q_i(A)$ will enable faster and smoother convergence in $\mathbf{r}_i$. Consider $Q_j(A)$ to be an "ansatz" or guess. We already know that $Q_j(A) = 1$ gives us back the "original" conjugate gradient method.


From $Q_{j+1}(A) = (1- \omega_j A) Q_j(A)$, Eq. \ref{Eq:BiconjugateGradientMethodPolynomialRecursion}, 
\[
\begin{gathered}
	\mathbf{r}_{j+1} = Q_{j+1}(A) P_{j+1}(A)\mathbf{r}_0 = (1- \omega_j A) Q_j(A) P_{j+1}(A) \mathbf{r}_0 = \sum_{k=0}^{j + 1} a^{j+1}_k A^{j + 1-k} P_{j+1}(A) \mathbf{r}_0 = (1 - \omega_j A) \sum_{k=0}^j a_k^j A^{j-k} P_{j+1}(A) \mathbf{r}_0 
\end{gathered}
\]
\begin{equation}
	\xrightarrow{ \mathbf{r}_0^T } a_0^{j+1} = -\omega_j a_0^j
\end{equation}

We'll compare a number of implementations of the algorithm for the biconjugate gradient stabilizied (BCGST).

Consider this implementation from Zeng, pp.5 Algorithm 2.3, in \href{https://utminers.utep.edu/xzeng/2017spring_math5330/MATH_5330_Computational_Methods_of_Linear_Algebra_files/ln07.pdf}{Lecture Note 7, Math 5330, UTEP}.

\begin{equation}
\begin{gathered}
	\begin{aligned}
	& \text{ Compute } \mathbf{r}_0 = \mathbf{b} - A \mathbf{x}_0, \text{ choose } \mathbf{r}_0' \text{ s.t. } \mathbf{r}_0 \cdot \mathbf{r}'_0 \neq 0, \text{ e.g. } \mathbf{r}_0' = \mathbf{r}_0 \\
	& \text{ Set } \mathbf{p_0} = \mathbf{r_0}
\end{aligned} \\
\begin{gathered}
	\forall \, j = 0, 1, \dots \\
	\begin{aligned}
		& \alpha_j = \frac{ (\mathbf{r}_0' \cdot \mathbf{r}_j) }{ \mathbf{r}_0' \cdot A \mathbf{p}_j } \\
		& \mathbf{s}_j = \mathbf{r}_j - \alpha_j A \mathbf{p}_j \\
		& \omega_j = \frac{ \mathbf{s}_j \cdot (A \mathbf{s}_j) }{ (A\mathbf{s}_j)\cdot (A\mathbf{s}_j) } \\
		& \mathbf{x}_{j+1} = \mathbf{x}_j + \alpha_j \mathbf{p}_j + \omega_j \mathbf{s}_j \\
		& \mathbf{r}_{j+1} = \mathbf{s}_j - \omega_j A\mathbf{s}_j \\
		& \text{ if } \| \mathbf{r}_{j+1} \| < \epsilon_0 \text{ then break. } \\
		& \beta_j = \frac{ \alpha_j }{ \omega_j } \frac{ (\mathbf{r}_0' \cdot \mathbf{r}_{j+1} ) }{ (\mathbf{r}_0' \cdot \mathbf{r}_j )} \\
		& \mathbf{p}_{j+1} = \mathbf{r}_{j+1} + \beta_j (\mathbf{p}_j - \omega_j A \mathbf{p}_j)
	\end{aligned} 
\end{gathered}	\\
\text{ Set } \mathbf{x} = \mathbf{x}_{j+1}
\end{gathered}
\end{equation}
	...
	\begin{gathered}
		...
		\begin{gathered}
			...
			\begin{aligned}
				& \alpha_j = \frac{ (\mathbf{r}_0' \cdot \mathbf{r}_j) }{ \mathbf{r}_0' \cdot A \mathbf{p}_j } \\
				& \mathbf{s}_j = \mathbf{r}_j - \alpha_j A \mathbf{p}_j \\
				& \text{ if } \| \mathbf{s}_j \| < \epsilon_0 \text{ , then } \\
				...
				& \beta_j = \frac{ \alpha_j }{ \omega_j } \frac{ (\mathbf{r}_0' \cdot \mathbf{r}_{j+1} ) }{ (\mathbf{r}_0' \cdot \mathbf{r}_j )} \\
				& \mathbf{p}_{j+1} = \mathbf{r}_{j+1} + \beta_j (\mathbf{p}_j - \omega_j A \mathbf{p}_j) \\
				& \text{ if } | \mathbf{r}_{j+1} \cdot \mathbf{r}_0' | < 10^{-6} \text{ then } \\
				& \quad \quad \, \begin{aligned}
					& \mathbf{r}_0' = \mathbf{r}_{j+1} \\
...
We will use this algorithm for the biconjugate gradient stabilizied (BCGST)
...
\begin{tikzpicture}[auto, node distance=1cm,>=latex']
            ...
            \node [block, right=of sum] (controller) {$G_1(s) +G_2(s)$};
            \node [output, right=of controller] (output) {};
            \node at ($(controller)!0.5!(controller)+(0,-2)$) [block] (feedback) {$G_3(s) - G_4(s)$};
            \draw [draw,->] (input) -- node {$U(s)$} (sum);
            \draw [->] (sum) -- (controller);
            \draw [->] (controller) -- node [name=y] {$C(s)$}(output);
            \draw [->] (y) |- (feedback) ;
            \draw [->] (feedback) -| node[pos=0.99] {$-$} (sum);
            \end{tikzpicture}    
\]

\problemhead{B-2-7 of Ogata (2009) \cite{Ogat2009}} pp. 61, Ch. 2 "Mathematical Modeling of Control Systems"
...
Y_p(s) + D(s) = C_D(s) \\
\Longrightarrow -G_c(s) G_p(s) C_D(s) = C_D(s) - D(s)
\end{gathered}
\]
so
\[
\boxed{
\frac{C_D(s)}{D(s) } = \frac{1}{ 1 + G_c(s) G_p(s) }
}
\]
...
\[
\Longrightarrow \boxed{ C(s) = C_R(s) + C_D(s) = \frac{ R(s) G_c(s) G_p(s) + D(s)}{ 1 + G_c(s) G_p(s) } }
\]
...
\section{Spacecraft Attitude Control}

\subsection{Reaction Wheels}

cf. Slide 21/45, M. Peet, Lecture 15.
...
\phantom{where} $\omega_f \equiv$ angular velocity ofh flywheel wih respect to craft. \\

So if craft has some angular velocity $\omega_s$ in $\widehat{b}_1$-direction (body-frame axis 1?) and reaction wheel is aligned with this axis, null out velocity by spinning up to


\section{More resources}

Daniel Block, Karl \"{A}str\"{o}m, Mark W. Spong. \textbf{The Reaction Wheel Pendulum}, i.e. Block, \"{A}str\"{o}m, and Spong (2007) \cite{BAS2007}


Kuo. \textbf{Digital Control Systems}.

Sidi. \textbf{Spacecraft Dynamics and Control}.
...
% cf. https://tex.stackexchange.com/questions/166781/control-system-diagram
...
Curtis (2020) \cite{Curt2020}

See pp. 54, Ch. 1 "Dynamics of Point Masses" of Curtis (2020) \cite{Curt2020}.

\problemhead{1.23} Use an RK solver to solve the nonlinear Lorenz equations, due to the American meteorologist and mathematician E.N. Lorenz (1917-2008):
\[
\begin{aligned}
& \dot{x} = \sigma( y- x ) \\
& \dot{y} = x(\rho - z) - y \\
...
So from \href{https://cdanfort.w3.uvm.edu/research/lorenz-1963.pdf}{"Deterministic Nonperiodic Flow"} by Edward N. Lorenz, 
...
So then
\begin{equation}\label{Eq:IntegrateConservedVector}
\begin{gathered}
...
\href{https://www.mathworks.com/matlabcentral/fileexchange/15285-geodetic-toolbox}{Mike Craymer (2023)'s Geodetic Toolbox in Matlab}. 
...
\bibitem{Tewa2007}
Ashish Tewari. \textbf{Atmospheric and Space Flight Dynamics: Modeling and Simulation with MATLAB and Simulink}. Birkh\"{a}user. 2007.
...
\bibitem{Curt2020}
Howard D. Curtis. \textbf{Orbital Mechanics for Engineering Students}. Butterworth-Heinemann. Fourth Edition. 2020.
...
\bibitem{BAS2007}
Daniel J. Block, Karl J. \"{A}str\"{o}m, Mark W. Spong. \textbf{The Reaction Wheel Pendulum}. Springer Cham. 2007.
