...
\quad \\ 
Mach number has a physical significance: \\
inertial force on a small fluid cube could be estimated by $F_{\text{in}} \simeq \rho d^2 u^2$ (inertial force is the force "trying to push fluid forward"). \\
For a perfect gas, the compressive, elastic force on fluid cube must be proportional to density, temperature, and area:
\[
F_{\text{el}} \propto \rho T d^2
\]
So the ratio of inertial to elastic or compressive forces on a fluid cube is
\[
\frac{F_{\text{in}}}{F_{\text{el}}} \simeq \frac{ \rho d^2 V^2 }{ \rho d^2 T} \simeq M^2
\]
Mach number can also be interpreted as the ratio of the time necessary for a pressure signal to travel a fixed distance to the time necessary for a fluid particle to cover the same distance.

\problemhead{9.1 Sabersky, et. al. (1998) \cite{SAHG1998}}
