...
optional<float> CuBLASVectorOperations::get_norm(const Array& x)
{
  HandleUnsuccessfulCuBLASCall handle_norm {"Failed to get norm in cuBLAS"};

  float result {0.0f};

  handle_norm(cublasSnrm2(
    cublas_handle_,
    x.number_of_elements_,
    x.values_,
    1,
  ...
  return handle_norm.is_cuBLAS_success() ? make_optional(result) : nullopt;
}

optional<float> CuBLASVectorOperations::get_norm(const DenseVector& x)
{
  HandleUnsuccessfulCuBLASCall handle_norm {"Failed to get norm in cuBLAS"};
...
DoubleCuBLASVectorOperations::DoubleCuBLASVectorOperations():
  cublas_handle_{0}
{
  HandleUnsuccessfulCuBLASCall handle_create_handle {
    "Failed to create cuBLAS handle"};
...
bool DoubleCuBLASVectorOperations::scalar_multiply_and_add_vector(
  ...
  DoubleArray& y)
{
  ...
  const double alpha {input_alpha};

  handle_vector_operations(cublasDaxpy(
    cublas_handle_,
    x.number_of_elements_,
    &alpha,
    // const double* x, vector with n elements.
    x.d_values_,
    // int incx - stride between consecutive elements of x.
    1,
    y.values_,
...
bool DoubleCuBLASVectorOperations::scalar_multiply(
  const double scalar,
  DenseVector& x,
  const std::size_t stride)
{
  ...
  handle_scalar_multiplication(cublasDscal(
    cublas_handle_,
    x.number_of_elements_,
    &alpha,
    x.d_values_,
...
optional<double> DoubleCuBLASVectorOperations::dot_product(
...
bool DoubleCuBLASVectorOperations::copy(const DoubleArray& x, DenseVector& y)
{
  HandleUnsuccessfulCuBLASCall handle_copy {"Failed copy in cuBLAS"};

  handle_copy(cublasDcopy(
    cublas_handle_,
    x.number_of_elements_,
    x.values_,
    1,
...
bool DoubleCuBLASVectorOperations::copy(const DenseVector& x, DenseVector& y)
{
  HandleUnsuccessfulCuBLASCall handle_copy {"Failed copy in cuBLAS"};
...
bool DoubleCuBLASVectorOperations::copy(const DenseVector& x, DoubleArray& y)
{
  HandleUnsuccessfulCuBLASCall handle_copy {"Failed copy in cuBLAS"};
...
optional<double> DoubleCuBLASVectorOperations::get_norm(const DoubleArray& x)
{
  HandleUnsuccessfulCuBLASCall handle_norm {"Failed to get norm in cuBLAS"};

  double result {0.0};

  handle_norm(cublasDnrm2(
    cublas_handle_,
    x.number_of_elements_,
    x.values_,
    1,
...
optional<double> DoubleCuBLASVectorOperations::get_norm(const DenseVector& x)
{
  HandleUnsuccessfulCuBLASCall handle_norm {"Failed to get norm in cuBLAS"};
