...
TEST(GenerateCompressedSparseRowMatrixTests, HasCorrectIndiciesForSquareMatrix)
{
  ...
  EXPECT_EQ(h_csr.J_[h_csr.number_of_elements_ - 5], GenerateCompressedSparseRow::M - 3);
  EXPECT_EQ(h_csr.J_[h_csr.number_of_elements_ - 4], GenerateCompressedSparseRow::M - 2);
  EXPECT_EQ(h_csr.J_[h_csr.number_of_elements_ - 3], GenerateCompressedSparseRow::M - 1);

  EXPECT_EQ(h_csr.J_[h_csr.number_of_elements_ - 2], GenerateCompressedSparseRow::M - 2);
  EXPECT_EQ(h_csr.J_[h_csr.number_of_elements_ - 1], GenerateCompressedSparseRow::M - 1);
}
